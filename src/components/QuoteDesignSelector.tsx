...
const QuoteDesignSelector: React.FC<QuoteDesignSelectorProps> = ({ 
  ...
  return (
    <div className="absolute top-4 right-4 z-10 font-inter">
      <div className="relative">
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="flex items-center gap-2 bg-white/70 backdrop-blur-sm px-4 py-2 rounded-full shadow-sm 
                   border border-gray-100 hover:bg-white/90 transition-all"
        >
          ...
          <div className="absolute right-0 mt-2 w-40 bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
            {designOptions.map((design) => (
              <button
                ...
                className={cn(
                  "w-full text-left px-4 py-2 text-sm hover:bg-pastel-blue/10 transition-colors",
                  design.id === currentDesign ? "bg-pastel-blue/20 font-medium" : ""
                )}
              >
                {design.name}
              </button>
