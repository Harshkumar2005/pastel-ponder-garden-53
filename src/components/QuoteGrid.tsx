...
const QuoteGrid: React.FC<QuoteGridProps> = ({
  ...
  const renderFilterBar = () => <div className="filter-bar bg-white/70 backdrop-blur-sm rounded-full mb-8 animate-fade-in ">
      <div className="flex flex-col sm:flex-row justify-between gap-4 bg-gray-50 hidden">
        {/* Left side - Search */}
        <div className="relative flex-grow max-w-2xl">
          <div className="relative flex w-full items-center">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
            <Input type="search" placeholder="Search quotes or tags..." value={localSearchQuery} onChange={e => setLocalSearchQuery(e.target.value)} className="pl-12 pr-4 py-2 w-full border border-gray-200 rounded-full font-inter text-sm focus:outline-none focus:ring-0 focus:border-pastel-blue" />
            {localSearchQuery && <Button variant="ghost" size="sm" className="absolute right-2 top-1/2 -translate-y-1/2 h-8 w-8 p-1 rounded-full hover:bg-gray-100" onClick={() => setLocalSearchQuery('')}>
        ...
        <div className="flex flex-wrap items-center justify-end gap-3">
          <div className="flex items-center gap-2">
            ...
            <Select value={moodFilter} onValueChange={setMoodFilter}>
              ...
              <SelectContent className="rounded-xl border border-gray-100 shadow-md bg-white overflow-hidden font-inter">
                ...
                {availableTags.map(tag => <SelectItem key={tag} value={tag} className="rounded-lg hover:bg-pastel-blue/10 focus:bg-pastel-blue/10">
                    {tag}
                  </SelectItem>)}
              </SelectContent>
            </Select>
          ...
          {activeFilters > 0 && <Button variant="outline" size="sm" onClick={resetFilters} className="text-xs text-gray-500 hover:text-gray-700 rounded-full border border-gray-200 hover:bg-gray-50 focus:outline-none focus:ring-0">
              Reset filters
            </Button>}
          
          {activeFilters > 0 && <Badge variant="outline" className="bg-pastel-blue/30 border-pastel-blue/40 rounded-full px-3 py-1 font-inter">
