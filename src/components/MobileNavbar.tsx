...
const MobileNavbar: React.FC = () => {
  ...
  return <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 shadow-lg rounded-t-xl px-2 py-1 md:hidden z-10">
      <div className="flex items-center justify-around max-w-md mx-auto">
        ...
        scale: 1.03
      }} className="flex flex-row items-center justify-center py-1.5 bg-pastel-blue/20 rounded-full border border-pastel-blue/30 shadow-sm transition-colors gap-2">
          <Dices className="h-5 w-5 text-blue-600" />
          <span className="text-xs font-medium text-blue-600">Random</span>
        </motion.button>
        
        <Link to="/bookmarks" className={cn("flex flex-col items-center justify-center px-3 py-1.5 rounded-lg transition-colors", isActivePath("/bookmarks") ? "text-black font-medium" : "text-gray-600")}>
          <BookmarkIcon className={cn("h-5 w-5 transition-colors", isActivePath("/bookmarks") ? "fill-current text-black" : "")} />
